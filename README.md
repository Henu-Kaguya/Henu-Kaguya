...
> ![qq群二维码](image/README.assets/qq_group_764079194.jpg)
...
- 雨课堂上课自动答题项目: [雨课堂项目](https://github.com/Zeshawn-z/ChangjiangRainClassroomAssistant)
  - Release:[Release](https://github.com/Zeshawn-z/ChangjiangRainClassroomAssistant/releases/tag/v1.0.11)
  - 主要能:自动监听课程并加入课程,自动下载ppt(老师还没放到那一页就可以下载,以及获取题目),获取题目之后接大模型API进行答题,提前生成答案,等答案发布退避一段时间进行答题,防止检测,以及导出题目等等功能
    - 现在只支持选择题,我已经没课了,只能靠你们贡献了
    - **注意**:自动加入课堂功能慎用,建议先扫码加入课堂之后再启动程序,因为如果是程序加入课堂,是跟用`雨课堂APP直接点击`是一样的,有的老师会要求**只能**`扫码加入`,后台能看到,这边建议社会工程学解决
  - 接大模型API之后能自动答题,支持qt和服务器模式,根据需要自行选择
  - 挂载服务器可实现多个人的登陆以及多个人的定时任务(把你课表设置定时任务就行)
    - 严禁售卖!包括但不限于帮别人挂载等等,出问题概不负责
- 还在开发雨课堂试卷导出,PPT导出项目
- 头歌相关题目导出也在开发中
